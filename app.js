...
function buildReceiptPreview(){
  ...
  document.getElementById('receipt-preview').innerHTML=`
    <div class="receipt">
      <div class="receipt-logo brand-lockup">
        <img src="assets/logo.svg" alt="InspectPro logo" class="brand-logo-icon">
        <span class="brand-name">Inspect<span>Pro</span></span>
      </div>
...
function renderInbox(){
  ...
  list.innerHTML=allJobs.map(({sub,subIdx,profile})=>{
    ...
    return `<div class="inbox-item ${sc}">
      ...
      ${status==='accepted'?`<div style="display:flex;gap:8px;margin-top:4px">
...
function startInspectionFromJob(profileId, subIdx) {
  // Start a full inspection linked to this customer's request
  const profile = custProfiles.find(p=>p.id===parseInt(profileId));
  if (!profile) return;
  const sub = profile.submissions[subIdx];

  state = {step:1, photos:[], checklist:{}, recServices:[], walkVideoBlob:null,
    walkStream:null, walkRecorder:null, walkChunks:[], linkedCustProfileId: profile.id};
  ...
  setTimeout(() => {
    ...
    setVal('f-vehicle', profile.vehicle);
    // Pre-select their requested services
    if (sub.requestedServices?.length) {
      state.recServices = [...sub.requestedServices];
      renderRecServices();
    }
  }, 100);
  ...
  setTimeout(() => {
    ...
    setVal('f-name', profile.name);
    setVal('f-phone', profile.phone);
    setVal('f-tech', currentTechAccount?.name || '');
    if (sub.requestedServices?.length) {
      state.recServices = [...sub.requestedServices];
      renderRecServices();
    }
  ...
  [1,2,3,4].forEach(n=>{ postJobState[n]=false; const el=document.getElementById('pj-'+n); if(el){el.classList.remove('checked');el.textContent='';} });
  buildChecklist(); gotoScreen('inspection'); goStep(1);
  toast('📋 Inspection started for ' + profile.name);
}
...
function sendToSearch(query){
  const results=document.getElementById('send-to-results'); if(!results) return;
  const q=(query||'').trim().toLowerCase();
  if(!q){results.innerHTML='';return;}
  const matches=techAccounts.filter(a=>a.name.toLowerCase().includes(q)||a.username.toLowerCase().includes(q));
...
function setSendToTech(username,name){
  ...
  const nameEl=document.getElementById('sts-name');
  const subEl=document.getElementById('sts-sub');
  const initials=name.split(' ').map(w=>w[0]||'').join('').toUpperCase().slice(0,2)||'?';
  if(avatar) avatar.textContent=initials;
  if(nameEl) nameEl.textContent=name;
  if(subEl)  subEl.innerHTML=`@${username} · ${getPortalStars(username)}`;
...
function clearSelectedTech(){
  ...
  const sr=document.getElementById('send-to-results');
...
function updateSubmitBtn(){
  ...
  if(portalSelectedTech){
    ...
    if(status){status.style.color='rgba(255,59,59,.7)';status.textContent='⚠ Select a technician to submit';}
  }
}

function searchTechnicians(query){
  const results=document.getElementById('tech-search-results'); if(!results) return;
  const q=(query||'').trim().toLowerCase();
  if(!q){results.innerHTML='';return;}
  const matches=techAccounts.filter(a=>a.name.toLowerCase().includes(q)||a.username.toLowerCase().includes(q));
  if(!matches.length){results.innerHTML=`<div style="font-size:13px;color:rgba(255,255,255,.35);padding:12px 0;text-align:center">No technician found for "${query}"</div>`;return;}
  const profile=custProfiles.find(p=>p.id===currentCustPortalId);
  const currentMech=profile?.preferredMechanic;
  results.innerHTML=matches.map(a=>{
    const initials=a.name.split(' ').map(w=>w[0]||'').join('').toUpperCase().slice(0,2)||'?';
...
function selectMechanic(username,name){
  const idx=custProfiles.findIndex(p=>p.id===currentCustPortalId);
  if(idx<0) return;
  custProfiles[idx].preferredMechanic=username; custProfiles[idx].preferredMechanicName=name;
  saveCustProfiles();
  searchTechnicians(document.getElementById('tech-search-input').value);
  toast('✓ '+name+' is now your mechanic!');
}

function renderCustomerCarReport(report){
  const container=document.getElementById('portal-car-report-content'); if(!container) return;
  const urgent=Object.entries(report.checklist||{}).filter(([,v])=>v==='r');
  const soon=Object.entries(report.checklist||{}).filter(([,v])=>v==='y');
  const good=Object.entries(report.checklist||{}).filter(([,v])=>v==='g');
  const irow=(label,icon,color,bg)=>`<div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.07)"><span style="font-size:16px">${icon}</span><span style="flex:1;font-size:13px;font-weight:600;color:#fff">${label}</span><span style="font-size:11px;font-weight:800;color:${color};background:${bg};padding:3px 10px;border-radius:20px">${icon==='🔴'?'Replace Now':icon==='🟡'?'Replace Soon':'Good'}</span></div>`;
  let html=`<div style="font-size:12px;color:rgba(255,255,255,.4);margin-bottom:14px;background:rgba(255,255,255,.04);border-radius:10px;padding:10px 12px">📅 ${new Date(report.date).toLocaleDateString()} · 🚗 ${report.vehicle||'—'} · <span style="color:var(--accent);font-weight:700">🔧 ${report.mechanicName||report.tech||'—'}</span></div>`;
  if(!urgent.length&&!soon.length) html+=`<div style="background:rgba(34,214,122,.15);border:1px solid rgba(34,214,122,.3);border-radius:12px;padding:14px;text-align:center;margin-bottom:14px"><div style="font-size:24px;margin-bottom:4px">✅</div><div style="font-family:'Syne',sans-serif;font-weight:800;font-size:16px;color:#22d67a">All Clear!</div></div>`;
  if(urgent.length) html+=`<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#ff3b3b;margin-bottom:6px">🔴 Replace Now</div><div style="background:rgba(255,255,255,.04);border-radius:10px;padding:0 12px">${urgent.map(([k])=>irow(k,'🔴','#ff3b3b','rgba(255,59,59,.12)')).join('')}</div></div>`;
  if(soon.length)   html+=`<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#ffb800;margin-bottom:6px">🟡 Replace Soon</div><div style="background:rgba(255,255,255,.04);border-radius:10px;padding:0 12px">${soon.map(([k])=>irow(k,'🟡','#ffb800','rgba(255,184,0,.12)')).join('')}</div></div>`;
  if(good.length)   html+=`<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#22d67a;margin-bottom:6px">🟢 Looking Good</div><div style="background:rgba(255,255,255,.04);border-radius:10px;padding:0 12px">${good.map(([k])=>irow(k,'🟢','#22d67a','rgba(34,214,122,.12)')).join('')}</div></div>`;
  if(report.workDone) html+=`<div style="background:rgba(34,214,122,.08);border:1px solid rgba(34,214,122,.2);border-radius:12px;padding:14px;margin-bottom:14px"><div style="font-size:11px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;color:#22d67a;margin-bottom:8px">✅ Work Completed</div><div style="font-size:13px;color:rgba(255,255,255,.75);line-height:1.7">${report.workDone}</div></div>`;
  if(report.comments) html+=`<div style="background:rgba(255,255,255,.05);border-radius:10px;padding:12px 14px;font-size:13px;color:rgba(255,255,255,.65);line-height:1.7">${report.comments}</div>`;
  container.innerHTML=html;
}

function renderPortalJobStatus(profile){
  const card=document.getElementById('portal-job-status-card');
  const content=document.getElementById('portal-job-status-content');
  if(!card||!content) return;
  const subs=(profile.submissions||[]).slice().reverse().filter(s=>s.sentToTechName);
  if(!subs.length){card.style.display='none';return;}
  card.style.display='block';
  content.innerHTML=subs.slice(0,3).map(sub=>{
    const status=sub.jobStatus||'pending';
    const sm={pending:{color:'#ffb800',bg:'rgba(255,184,0,.09)',border:'rgba(255,184,0,.25)',icon:'⏳',text:'Awaiting Response'},accepted:{color:'#22d67a',bg:'rgba(34,214,122,.09)',border:'rgba(34,214,122,.25)',icon:'✅',text:'Job Accepted'},declined:{color:'#ff3b3b',bg:'rgba(255,59,59,.07)',border:'rgba(255,59,59,.2)',icon:'❌',text:'Job Declined'}};
    const s=sm[status]||sm.pending;
    return `<div style="background:${s.bg};border:1.5px solid ${s.border};border-radius:12px;padding:14px;margin-bottom:10px"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><span style="font-size:20px">${s.icon}</span><div><div style="font-weight:700;font-size:14px;color:${s.color}">${s.text}</div><div style="font-size:11px;color:rgba(255,255,255,.4);margin-top:2px">${new Date(sub.date).toLocaleString()}</div></div></div><div style="font-size:13px;color:rgba(255,255,255,.65)">To: <strong style="color:#fff">${sub.sentToTechName}</strong></div></div>`;
  }).join('');
}
...
function showCardModal(onSuccess) {
  ...
  modal.innerHTML = `
    <div style="background:#252540;border:1px solid rgba(232,255,71,.2);border-radius:20px;padding:28px;width:100%;max-width:400px">
      ...
      <div id="modal-card-error" style="display:none;background:rgba(255,59,59,.1);border:1px solid rgba(255,59,59,.25);border-radius:8px;padding:10px;font-size:12px;color:#ff6b6b;margin-bottom:14px"></div>
      <div style="display:flex;gap:8px">
        <button onclick="closeCardModal()" style="flex:1;padding:12px;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.12);border-radius:10px;color:rgba(255,255,255,.5);font-family:'Mulish',sans-serif;font-size:13px;font-weight:700;cursor:pointer">Cancel</button>
        <button id="modal-pay-btn" onclick="saveCardAndSubmit()" style="flex:2;padding:12px;background:#e8ff47;border:none;border-radius:10px;color:#1a1a2e;font-family:'Syne',sans-serif;font-size:15px;font-weight:800;cursor:pointer">Save Card & Submit</button>
      </div>
      ...
      <div style="font-size:10px;color:rgba(255,184,0,.6);text-align:center;margin-top:8px">Test card: 4242 4242 4242 4242 · any future date · any CVC</div>
    </div>`;
  document.body.appendChild(modal);
  ...
  if (stripe) {
    ...
    // Stripe.js not loaded — skip card for now (dev/test mode)
    document.getElementById('modal-card-element').innerHTML =
      '<div style="font-size:13px;color:rgba(255,255,255,.4)">Stripe not loaded — add Stripe.js script to index.html to enable payments.</div>';
  }
    ...
    const save = (videoDataUrl) => {
      ...
      if (content) content.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;padding:64px 24px;text-align:center">
          ...
          <button onclick="logout()" style="background:rgba(255,255,255,.08);border:1.5px solid rgba(255,255,255,.15);border-radius:10px;color:#fff;font-family:'Mulish',sans-serif;font-size:14px;font-weight:700;padding:12px 32px;cursor:pointer">Close & Exit</button>
        </div>`;
    };
...
// ── Charge booking fee when tech accepts ──
